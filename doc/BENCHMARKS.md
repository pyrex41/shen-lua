# shen-lua benchmarks (Shen 42)

All numbers below are from the current 42 port running under
...
this file for context. shen-c does not build for 42 and was not
re-measured here, so the *primary* numbers below are absolute
shen-lua-on-LuaJIT performance, not a head-to-head.

---

## 1. Cold startup (Shen 42)

| | mean of 5 runs |
|---|---:|
| shen-lua (42) | **~0.58 s** (load 0.56 s + initialise 0.02 s) |

The 42 kernel is substantially larger than 22.4 (21 `.kl` files including
...
shen-c at 1.24 s / solve. The 42 port is ~4× faster than the old 22.4
...
## 4. Pass rate (Shen 42 official suite)
...
## 5. Compiler improvements that paid for 42

The 42 stlib and Prolog code stress LuaJIT's parser limits much harder
...
# ../cl-source/ShenOSKernel-42/tests):
...
NOT directly comparable to the 42 numbers above.
...
allocation and CPS closure churn). The 42 port now wins on Einstein
