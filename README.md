...
| [`examples/pcr/`](examples/pcr/) | **proof-carrying tool calls — who authorized your agent?**: a human delegates to an agent, the agent attenuates (read-only, enforced by the type system) to the subagent it spawns; each tool call carries its delegation chain as a proof term the OpenResty gate *checks* — never searches — against a versioned live fact store, so revoking one edge kills the whole subtree mid-run while every allow logs the exact authority that justified it ([README](examples/pcr/README.md)) |
